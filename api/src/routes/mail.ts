...
mailRouter.get('/inbox', async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message: '받은메일함을 불러오는 중 오류가 발생했습니다.',
    });
  }
});
...
mailRouter.get('/sent', async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message: '보낸메일함을 불러오는 중 오류가 발생했습니다.',
    });
  }
});
